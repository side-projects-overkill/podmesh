...
impl<'a> NetworkService<'a> {
    ...
    pub async fn list(&self) -> Result<Vec<Value>> {
        let (status, body) = self
            .request("GET", &format!("{API}/networks/json"), None)
            .await?;
    ...
    pub async fn inspect(&self, name: &str) -> Result<Value> {
        let (status, body) = self
            .request("GET", &format!("{API}/networks/{name}/json"), None)
            .await?;
        self.parse_response(status, &body)
    }
    ...
    pub async fn create(&self, spec: &Value) -> Result<Value> {
        ...
        let (status, resp) = self
            .request("POST", &format!("{API}/networks/create"), Some(&body))
            .await?;
    ...
    pub async fn remove(&self, name: &str, force: bool) -> Result<()> {
        let (status, body) = self
            .request(
                "DELETE",
                &format!("{API}/networks/{name}?force={force}"),
                None,
            )
            .await?;
        if status >= 300 {
