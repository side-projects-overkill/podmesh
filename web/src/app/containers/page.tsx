        ...
        {isLoading ? (
          ...
          <div className="divide-y divide-[var(--border)]">
            {data.map((c: Record<string, unknown>, i: number) => (
              <div key={i} className="grid grid-cols-6 gap-4 p-3 text-sm hover:bg-[var(--muted)]/50 transition-colors">
                <div className="font-medium truncate">{String(c.name ?? "—")}</div>
                <div className="truncate text-[var(--muted-foreground)]">{String(c.image ?? "—")}</div>
                <div>
                  <span className="rounded-full px-2 py-0.5 text-xs font-medium bg-[var(--muted)] text-[var(--muted-foreground)]">
