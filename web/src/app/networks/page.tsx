    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[var(--muted-foreground)]">Manage container networks</p>
        </div>
      ...
      <div className="rounded-lg border border-[var(--border)] bg-[var(--card)]">
        ...
        {isLoading ? (
          ...
          <div className="divide-y divide-[var(--border)]">
            {data.map((n: Record<string, unknown>, i: number) => (
              <div key={i} className="grid grid-cols-5 gap-4 p-3 text-sm hover:bg-[var(--muted)]/50 transition-colors">
                ...
                <div className="text-[var(--muted-foreground)]">{String(n.driver ?? "bridge")}</div>
                <div className="text-[var(--muted-foreground)]">—</div>
                <div className="text-[var(--muted-foreground)]">{String(n.node_id ?? "—")}</div>
          ...
          <div className="p-8 text-center text-sm text-[var(--muted-foreground)]">No networks found.</div>
        )}
      </div>
    </div>
  );
